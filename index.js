const axios = require('axios')
let _ = require('lodash')
const aguid = require('aguid')
const { v1: uuidv1 } = require('uuid')
const hasha = require('hasha')
var fs = require("fs");
var path = require("path");
const FormData = require('form-data')
const Promise = require('bluebird')

module.exports = function (mikser, context) {
    let config = mikser.config['whitebox']
    let options = _.defaultsDeep(config || {
        services: {
            data: {
                url: 'https://data.whitebox.pro'
            },
            storage: {
                url: 'https://storage.whitebox.pro'
            }
        }
    })
    
    if (!options.services.data.token) {
        console.error('WhtieBox data token is missing.')
        return Promise.resolve()
    }
    
    function api(service, url, data) {
        return axios.post(options.services[service].url + url + '?v=' + Date.now(), data, {
            headers: {
                'Authorization': 'Bearer ' + options.services[service].token
            }
        }).then(response => {
            if (response.data.success) {
                return Promise.resolve(response.data)
            } else {
                console.error('Api service error:', url, data, response.data.message)
                return Promise.reject(response.data.message)
            }
        }).catch(err => {
            console.error('Api system error:', url, data, err)
            return Promise.resolve()
        })
    }
    
    function upload(file) {
        let sourceFile = path.join(mikser.config.outputFolder, file)
        if (!fs.existsSync(sourceFile)) return Promise.resolve()
        console.log('Checking md5:', file)
        return axios.post(options.services.storage.url + '/' + options.services.storage.token + '/hash', { 
            file,
        })
        .then(response => {
            return hasha.fromFile(sourceFile, { algorithm: 'md5' }).then(hash => {
                console.log('MD5:', hash, response.data.hash)
                if (!response.data.success || hash != response.data.hash) {
                    console.log('Uploading:', file)
                    let form = new FormData()
                    form.append(file, fs.createReadStream(sourceFile))
                    const submit = Promise.promisify(form.submit, {context: form})
                    return submit(options.services.storage.url + '/' + options.services.storage.token + '/upload').then(res => {
                        console.log('Uploaded:', file, res.statusCode, res.statusMessage)
                        res.resume()
                    }).catch(err => console.error('Error uplaoding:', err))
                }
            })
        })
    }
    
    function unlink(file) {
        return axios.post(options.services.storage.url + '/' + options.services.storage.token + '/unlink', { 
            file, 
        })
        .catch(err => console.error('Error unlinking:', err))
    }
    
    if (context) {

    } else {
        mikser.on('mikser.manager.importDocument', (document) => {
            if (!document.meta.type && !document.meta.layout) return Promise.resolve()
            let data = {
                passportId: uuidv1(),
                vaultId: aguid(document._id),
                refId: document.destination,
                type: document.meta.type || document.meta.layout,
                data: document, 
                stamp: document.stamp,
                date: document.mtime,
            }
            return api('data', '/api/vault/keep/one', data, options)
        })
        mikser.on('mikser.manager.deleteDocument', (document) => {
            let data = {
                vaultId: aguid(document._id),
            }
            return api('data', '/api/vault/remove', data, options)
        })

        let throttle = {}
        mikser.on('mikser.watcher.outputAction', (event, file) => {
            clearTimeout(throttle[file])
            if (file.indexOf('/assets/') == -1) return Promise.resolve()
            console.log('WhiteBox storage sync:', event, file)
            
            setTimeout(() => {
                if (event == 'add') {
                    upload(file)
                } else if (event == 'unlink'){
                    unlink(file)
                } else if (event == 'change') {
                    upload(file)
                }
                delete throttle[file]
            }, 1000)
        })
        if (options.clear) {
            return api('data', '/api/vault/clear', {}).then(() => {
                return axios.post(options.services.storage.url + '/' + options.services.storage.token + '/clear', {})
            })
            .catch(err => console.error('Error clearing:', err))
        }
    }
}